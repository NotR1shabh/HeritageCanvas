document.addEventListener('DOMContentLoaded', () => {
  function normalizeAssetPath(assetPath) {
    ...
    if (/^(https?:)?\/\//iu.test(assetPath)) return assetPath;
    if (assetPath.startsWith('/')) return assetPath;
    if (assetPath.startsWith('images/') || assetPath.startsWith('filters/')) return `/${assetPath}`;
    return assetPath;
  }
  ...
  function renderEpicCard(index) {
    ...
    const imgSrc = normalizeAssetPath(data.image || data.front);
  ...
  function displayDetails(item) {
    ...
    if (item.image_url) {
      detailsImage.src = normalizeAssetPath(item.image_url);
    ...
    try {
      ...
      if (item.category === 'tales_and_epics' && epLower && epicCovers[epLower]) {
        ...
        if (isGeneric || detailsImage.src.endsWith('heritage_icon.png')) {
          detailsImage.src = normalizeAssetPath(epicCovers[epLower]);
          detailsImage.style.display = 'block';
        }
      }
    } catch (e) {
  ...
  function showMarkers() {
    ...
    (sites || []).forEach(item => {
      ...
      if (matchesTheme && matchesYear && matchesSearch) {
        ...
        popupImageUrl = normalizeAssetPath(popupImageUrl);
